...
public class GoldenAppleBlock extends AppleBlock{
    ...
    protected ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, BlockHitResult hit) {
        ...
        if (stack.isOf(Items.GOLDEN_APPLE) && state.get(APPLES) < 4) {
    ...
    protected static ActionResult tryEat(WorldAccess world, BlockPos pos, BlockState state, PlayerEntity player) {
        if (!player.canConsume(true)) {
            ...
            player.addStatusEffect(new StatusEffectInstance(StatusEffects.ABSORPTION, 2400), player);
            player.addStatusEffect(new StatusEffectInstance(StatusEffects.REGENERATION, 100, 1), player);
